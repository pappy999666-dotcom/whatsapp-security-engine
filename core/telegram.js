...
function extractYoutubeVideoId(url) {
    ...
    return raw.match(/[?&]v=([^&]+)/)?.[1]
        || raw.match(/youtu\.be\/([^?&]+)/)?.[1]
        || raw.match(/youtube\.com\/(?:shorts|live)\/([^?&/]+)/)?.[1]
        || raw;
}
...
async function downloadAndSendSong(ctx, url, statusMsg, queryHint, wantVideo = false) {
    if (/youtu\.be|youtube\.com/i.test(String(url || ''))) {
        ...
        const { downloadAudio, downloadVideo, getVideoInfo } = require('./youtube');
        ...
        const dl = wantVideo ? await downloadVideo(videoId, 48 * 1024 * 1024) : await downloadAudio(videoId);
        const title = dl.title || info?.title || queryHint || 'media';
        const artist = dl.uploader || info?.uploader || '';
        const duration = dl.duration || info?.duration || '';
        await editStatus(ctx, statusMsg, `${wantVideo ? '🎬' : '🎵'} <b>${escapeHtml(title)}</b>
📤 <i>Uploading...</i>`);
        if (wantVideo) {
            await ctx.replyWithVideo(
                { source: dl.buffer, filename: `${title}.${dl.fileExt || 'mp4'}` },
...
🎤 ${escapeHtml(artist)}` : ''}${duration ? `  ⏱ ${duration}` : ''}`, parse_mode: 'HTML', supports_streaming: true }
            ...
            await ctx.replyWithAudio(
                { source: dl.buffer, filename: `${title}.${dl.fileExt || 'mp3'}` },
                {
                    title,
                    performer: artist,
                    caption: `🎵 <b>${escapeHtml(title)}</b>${artist ? `
...
async function downloadUrlAndSend(ctx, url, statusMsg) {
    if (/youtu\.be|youtube\.com/i.test(String(url || ''))) {
        return downloadAndSendSong(ctx, url, statusMsg, null, true);
    }
