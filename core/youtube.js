'use strict';
// core/youtube.js — YouTube search + download facade
// Primary: @zeppeliorg/yt-sdl. Fallback: youtubei.js / yt-dlp for legacy deployments.
...
const ytSdl = require('./ytSdlClient');
...
async function searchYoutube(query, limit = 5) {
    try {
        const fromYtSdl = await ytSdl.search(query, limit);
        if (fromYtSdl.length) return fromYtSdl;
    } catch {}
...
async function downloadAudio(videoId) {
    ...
    try {
        const dl = await ytSdl.download(videoUrl, 'audio');
        if (dl?.buffer) return {
            buffer: dl.buffer,
            title: dl.title || videoId,
            uploader: dl.uploader || 'Unknown',
            duration: dl.duration || '?',
            thumb: dl.thumb || null,
            mimetype: dl.mimetype || 'audio/mpeg',
            fileExt: dl.fileExt || 'mp3',
        };
    } catch {}
...
async function downloadVideo(videoId, maxBytes = 48 * 1024 * 1024) {
    const videoUrl = `https://www.youtube.com/watch?v=${videoId}`;
    try {
        const dl = await ytSdl.download(videoUrl, 'video');
        if (dl?.buffer) {
            if (dl.buffer.length > maxBytes) throw new Error('Video too large (>48MB)');
            return {
                buffer: dl.buffer,
                title: dl.title || videoId,
                uploader: dl.uploader || 'Unknown',
