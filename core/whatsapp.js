...
async function resolveSongPollSelections(pollUpdate, cachedPoll, ctx = {}) {
    const pollMessage = cachedPoll?.message || ctx.pollMessage || null;
    ...
    if (ctx.optionHashMap && typeof ctx.optionHashMap === 'object') {
        const mapped = [];
        for (const hash of selectedHashes) {
            const hashKey = Buffer.isBuffer(hash) ? hash.toString('base64') : String(hash);
            if (ctx.optionHashMap[hashKey]) mapped.push(ctx.optionHashMap[hashKey]);
        }
    ...
    sock.ev.on('messages.upsert', async ({ messages, type }) => {
        ...
        for (const m of messages) {
            try {
                ...
                const pollLookup = pollCreationKey ? global._songPollLookup?.get(pollCreationKey) : null;
                const cachedPoll = global.messageCache?.get(pollCreationKey) || (pollLookup?.pollMessage ? { message: pollLookup.pollMessage } : null);
                ...
                const selectedNames = (await resolveSongPollSelections(m.message.pollUpdateMessage, cachedPoll, {
                    ...
                    voterJid,
                    pollCreatorJid: pollLookup?.pollCreatorJid || sock.user?.id,
                    pollMessage: pollLookup?.pollMessage || null,
                    optionHashMap: pollLookup?.optionHashMap || null,
                    messageSecret: pollLookup?.messageSecret,
                }))
