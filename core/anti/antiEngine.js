...
// ─── Result builder ───────────────────────────────────────────────────────────
function _match(module, reason, action = 'delete') {
...
function processAntiChecks(ctx, botId) {
    ...
    for (const check of PIPELINE) {
        try {
            ...
            if (result.triggered) {
                const configuredAction = cfg[`${result.module}Action`];
                if (['kick', 'warn', 'delete'].includes(configuredAction)) result.action = configuredAction;
                return result;
            }
        } catch (err) {
