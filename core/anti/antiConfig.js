...
const ADMIN_ACTION_MAP = {
    ...
    antibotAction:     'antiBotAction',
    antispamAction:    'antiSpamAction',
    antichannelAction: 'antiCBroadcastAction',
    antidemoteAction:  'antiPromoteAction',
    antigstatusAction: 'antiGStatusAction',
};
...
function _getAdminBridgedFlags(groupJid, botId) {
    ...
    for (const [adminKey, engineKey] of Object.entries(ADMIN_ACTION_MAP)) {
        const action = String(group[adminKey] || '').toLowerCase();
        if (['kick', 'warn', 'delete'].includes(action)) bridged[engineKey] = action;
    }
    if (Number.isFinite(Number(group.maxWarns))) bridged.maxWarns = Number(group.maxWarns);
    return bridged;
}
